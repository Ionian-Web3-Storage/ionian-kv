...
use config::IonianKVConfig;
use std::error::Error;

async fn start_node(context: RuntimeContext, config: IonianKVConfig) -> Result<Client, String> {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let config = IonianKVConfig::parse(&matches)?;
    ...
    executor.clone().spawn(
        async move {
            ...
            if let Err(e) = start_node(context.clone(), config).await {
                error!(reason = %e, "Failed to start ionian kv node");
                ...
                let _ =
                    executor
                        ...
                        .try_send(task_executor::ShutdownReason::Failure(
                            "Failed to start ionian kv node",
                        ));
            } else {
