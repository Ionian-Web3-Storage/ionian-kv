...
impl StreamReplayer {
    ...
    async fn replay(&self, tx: &Transaction) -> Result<bool> {
        ...
        let stream_read_set = self.parse_stream_read_set(&mut stream_reader).await?;
        if !(self
        ...
        let stream_write_set = self.parse_stream_write_set(&mut stream_reader).await?;
        if !(self
        ...
        let mut access_control_set = self
